...
impl NospaceVM {
    ...
    pub(super) fn step_eval_expr(&mut self) -> ExecuteResult {
        ...
        match cont {
            ...
            EvalCont::AfterUnary(op) => {
                ...
                let r =
                    crate::base::pure_eval::eval_unary_pure(&op, v).expect("unsupported unary op");
                self.finish_eval(r)
            }
            EvalCont::DerefAfter => {
            ...
            EvalCont::LogicalAndRhs(rhs) => {
                ...
                if left == 0 {
                    ...
                    self.set_eval_cont(EvalCont::BinaryRight {
                        op: Operator2::LogicalAnd,
                        left,
                    });
                    self.frames.push(Frame::EvalExpr {
                        expr: rhs,
    ...
    pub(super) fn eval_start(&mut self, expr: &LocatedExecExpression) -> ExecuteResult {
        match &expr.expression {
            ...
            ExecExpression::Operation2(op, lhs, rhs) => match op {
                Operator2::LogicalAnd => {
                ...
                _ => {
                    let lp: ExprPtr = lhs.as_ref() as *const _;
                    let rp: ExprPtr = rhs.as_ref() as *const _;
                    self.set_eval_cont(EvalCont::BinaryLeft {
                        op: op.clone(),
                        rhs: rp,
                    });
                    self.frames.push(Frame::EvalExpr {
            ...
            ExecExpression::If(mode, cond, then_block, else_block) => {
                ...
                self.set_eval_cont(EvalCont::IfCond {
                    mode: *mode,
                    then_block: tp,
                    else_block: ep,
                });
    ...
    pub(super) fn push_assign(
        &mut self,
        lhs: &LocatedExecExpression,
        rhs: &LocatedExecExpression,
    ) -> ExecuteResult {
        ...
        match &lhs.expression {
            ...
            ExecExpression::ArrayAccess(id_ref, index_expr, _) => {
                let ip: ExprPtr = index_expr.as_ref() as *const _;
                let id = *id_ref;
                self.set_eval_cont(EvalCont::AssignArrIndex {
                    id_ref: id,
                    rhs: rp,
                });
                self.frames.push(Frame::EvalExpr {
    ...
    pub(super) fn set_eval_cont(&mut self, cont: EvalCont) {
        if let Some(Frame::EvalExpr { cont: c, .. }) = self.frames.last_mut() {
            *c = cont;
        }
    }
}
