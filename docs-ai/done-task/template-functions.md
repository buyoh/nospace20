...
| Step | 内容 | 依存 | 状態 |
|------|------|------|------|
| 1 | AST 定義の拡張（テンプレート関数ノード） | なし | ✅ 実装済み |
| 2 | tree_parser: テンプレート関数定義のパース | Step 1 | ✅ 実装済み |
| 3 | tree_parser: テンプレートインスタンス化文のパース | Step 1 | ✅ 実装済み |
| 4 | semantic_analyzer: テンプレート収集・インスタンス化 | Step 2, 3 | ✅ 実装済み |
| 5 | テストケース追加 | Step 4 | ✅ 実装済み |
| 6 | spec.md / grammar.bnf への反映 | Step 4 | ✅ 実装済み |
...
- [docs-ai/architecture/overview.md](../architecture/overview.md) - アーキテクチャ概要

---

## 更新履歴

- 2026-03-01: テンプレート関数の全ステップ実装完了。テスト15件追加（success_io 9件、compile_error 6件）、spec.md/grammar.bnf 更新
