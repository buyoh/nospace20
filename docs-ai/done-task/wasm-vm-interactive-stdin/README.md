...
- [x] `StepResult::WaitingForInput` バリアントの追加
- [x] `ExecuteResult::WaitingForInput` バリアントの追加
- [x] stdin を追記可能なバッファ（`InteractiveBuffer`、`VecDeque` ベース）の実装
- [x] `InteractiveBuffer` の EOS（ストリーム終端）サポート（`close()` メソッド）
- [x] `InputChar` / `InputNumber` 命令の分岐: バッファ不足時に WaitingForInput、EOS 時に EOF を返す
- [x] `WhitespaceVM::with_interactive_stdin()` ビルダーメソッドの追加
- [x] ユニットテスト
...
- [x] `VmStepResult` TypeScript 型に `"waiting_for_input"` ステータスと `inputType` フィールド追加
- [x] `WasmWhitespaceVM::provide_stdin(data: &str)` メソッドの追加
- [x] `WasmWhitespaceVM::close_stdin()` メソッドの追加（EOS 通知）
- [x] コンストラクタで interactive stdin を利用するモードの追加
- [x] `VmStepResult` のシリアライズ更新
