...
- [x] コミット完了
- [x] ドキュメント移動（完了後 → `docs-ai/done-task/`）
...
詳細は `docs-ai/task/compiler-ws-duplicate-label-bug.md` に記載。
...
- 過去の修正: `docs-ai/done-task/fix-ws-self-label-duplication.md` (compiler_ws のラベル ID 重複バグ)
