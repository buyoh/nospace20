...
- `arr[i]` は、`*(&arr + i)` と同義。同様に、`(*p)[i]` は `*(p + i)` と同義。
...
- `&final_var` で参照を取得することは可能。参照を経由して値を変更できる。
...
- 識別子の衝突を防ぐための名前空間を作成できる
- ブロック末尾にセミコロンは不要
- 変数のライフタイムに影響しない（スコープは namespace 内のブロックと同じ）
- `MySpace$x` のようにアクセスする
...
- `@` に続けて型を指定することで、関数引数や変数の型を宣言できる
- 式に型注釈を付けることができる
- `@void` での変数宣言はコンパイルエラー

```
func: foobar(a @int, b @int) @void {
  __puti( ( a + b) @ int );
}
let: x@int(1);              # int 型変数 #
...
- void 型にキャストすると、式の値を捨てる（void 式になる）
- 整数型と参照型の相互キャストが可能
- 構造体はキャスト不可
...
- 宣言及び初期化は `struct: Name(values...)` の形式
- 構造体は代入や演算・関数の引数・戻り値としての受け渡しは不可
- `.` でフィールドにアクセスする
  - 参照に対しても `.` でアクセスできる（参照を自動的にデリファレンスする）。参照の参照は不可

```
let: s@MyStruct(struct: MyStruct(10, [1,2,3,4,5,6,7,8,9]));
let: t(struct: MyStruct(10, [1,2,3,4,5,6,7,8,9]));  # 型注釈省略 #
s.number = 10;
s.data[i] = 42;
let: p(&s);
p.number = 20;  # s.number が 20 に変更される #
```
...
型のスロットサイズを返すコンパイル時定数式。`void` は 0
