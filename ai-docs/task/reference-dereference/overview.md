...
インタプリタでは変数が `scope_stack: Vec<Vec<i64>>` と `global_variables`（`VariableStorage` トレイトで抽象化）に格納されている。
参照値を i64 にエンコードするスキームが必要。

`global_variables` は map 型（`BTreeMap<usize, i64>` 等）で実装するが、`VariableStorage` トレイトで抽象化し、パフォーマンスチューニング時に `Vec<i64>` 等へ切り替え可能にする。
...
**簡易方式（推奨）**: 全変数を統一的なフラットアドレス空間にマッピングする。`VariableStorage::len()` を使い、内部実装に依存しない。
