...
| `fsba_class_reuse_001` | FSBA: 同一サイズクラスの alloc/free/alloc で再利用されること |
| `fsba_different_class_001` | FSBA: 異なるサイズクラスのブロックが独立したフリーリストで管理されること |
| `fsba_roundup_001` | FSBA: サイズ切り上げが正しく動作すること (alloc(2)→4セルブロック, alloc(5)→8セルブロック) |
| `fsba_large_fallback_001` | FSBA: 32セルを超える割り当てが汎用アロケータにフォールバックすること |
...
| 高 | FSBA クラス分け・再利用 | 二層アーキテクチャの正しさ |
