...
|------|----------------|---------------------|
| 変数読み取り（実行時） | O(depth × log n) | **O(1)** ✅ |
| 変数書き込み（実行時） | O(depth × log n) | **O(1)** ✅ |
| 関数引数初期化（実行時） | O(args × log n) | **O(args)** ✅ |
| 変数解決（解析時） | - | O(depth × log n) (1回のみ) |
| メモリ | 変数名を複製 | インデックスのみ |
...
### 課題4: 関数引数初期化のパフォーマンス（追加最適化）

**問題**: 関数呼び出し時に引数名から変数インデックスを検索するため O(args × log n)

**解決**: `arg_indices: Vec<usize>` フィールドを追加
- 意味解析時に引数のインデックスを事前計算
- インタプリタで事前計算されたインデックスを使用
- 関数呼び出し時の引数初期化が **O(args)** に改善
