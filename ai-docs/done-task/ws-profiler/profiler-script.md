...
- `src/whitespace/interpreter.rs` に `ProfileStats`, `InstructionCounts`, `HeapProfileStats`, `StackProfileStats` 構造体を追加
- `WhitespaceVM` に `with_profiling(bool)`, `profile_stats()` メソッドを追加
- `src/whitespace/mod.rs` から新型をエクスポート
- `resources/tests/profile-targets.yaml` を作成（15テストケース定義）
- `examples/ws_profiler.rs` を作成（YAML 出力形式のプロファイラ）
- `Cargo.toml` に `[dev-dependencies] serde_yaml = "0.9"` と `[[example]] ws_profiler` を追加
- 全テストケース Complete で正常動作確認済み
